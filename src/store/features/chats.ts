...
import { IChatsChat, IChatsMessage, IChatsChatListResponse, IChatsGetChatInfoResponse, IChatsGetChatMessagesResponse, IChatsUploadFileLink } from "@app/models/chats";
...
export const chatsSlice = createSlice({
  ...
  reducers: {
    ...
    addUploadFiles: (state, action: PayloadAction<{ chat_id: string, files: Array<IChatsUploadFileLink> }>) => {
    ...
    deleteUploadFiles: (state, action: PayloadAction<{ chat_id: string, target_id: string }>) => {
      state.uploads[action.payload.chat_id] = state.uploads[action.payload.chat_id]
        .filter((file) => file.upload_id !== action.payload.target_id);
    },

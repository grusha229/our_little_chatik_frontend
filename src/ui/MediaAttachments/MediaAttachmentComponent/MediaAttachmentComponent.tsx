...
export default function MediaAttachmentComponent({
    ...
    const AttachmentComponent = getAttachmentComponent({
        content_type,
        preview_link,
        isFileUploaded,
        size,
    ...
    const blockClassName = buildClassName(styles['block'], styles[`block--${size}`]);

    const fileNameClassName = buildClassName(styles['file'], styles[`file--${size}`]);
        ...
        <div className={blockClassName} onClick={handleAttachmentClick}>
            ...
            {size !== 'large' && (
                <div className={fileNameClassName}>
                    <div className={styles['file-name']}>{file_name}</div>
                    <div className={styles['file-type']}>{getFileType(content_type)}</div>
                </div>
            )}
        </div>
    );
